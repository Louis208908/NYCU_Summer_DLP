...
class layers:
    def __init__(self,input_dim,output_dim, layer_name, act_fcn_type, data_type):
        ...
        self.previous_gradient_matrix = np.zeros((self.input_dim, self.output_dim));
        ...
        if (act_fcn_type == "relu" or act_fcn_type == "leaky relu") and data_type == "xor":
            ...
            self.self_bias *= 0.1
        if (act_fcn_type == "tanh" or act_fcn_type == "none"):
...
class activations:
    ...
    def forward_activation(self, input_z):
        if self.type == "sigmoid":
            ...
            self.stored_for_backward = np.divide(np.exp(input_z) - np.exp(-input_z), (np.exp(input_z) + np.exp(-input_z)))
            return self.stored_for_backward
...
class NN:
    ...
    def forward_propagate(self , input):                #x -> z -> a ->z -> a -> z -> output
        ...
        self.activated_output1 = self.input_layer.activation_fcn.forward_activation(self.output1);

        self.output2 = self.hidden_layer1.forward_output(self.activated_output1);

        self.activated_output2 = self.hidden_layer1.activation_fcn.forward_activation(self.output2);

        self.output3 = self.hidden_layer2.forward_output(self.activated_output2);

        self.activated_output3 = self.hidden_layer2.activation_fcn.forward_activation(self.output3)
    ...
    def backward_propagate(self , loss):
        ...
        self.hidden_layer1.activation_fcn
    ...
    def train(self, epoch, inputX,inputY):
        ...
        for i in range(epoch):
            ...
            epochs.append(i + 1)
            if (i + 5000) % 5000 == 0:
                print("epoch {:5d}, loss: {:.4f}, accuracy: {:.4f}".format(i + 5000, loss, accuracy));
