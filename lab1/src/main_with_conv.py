...
class layers:   #it's fully connected layer
...
class convolutional:
    ...
    def backward_propagate(self, loss):
        ...
        for i in range(row):
            for j in range(col - self.kernel_size[1] + 1):
                # output_grad_for_next_layer[i][j] = np.matmul(input_padded[i][j:j + self.kernel_size[1]], flipped_kernel.T);
                output_grad_for_next_layer[i][j] = input_padded[i][j:j + self.kernel_size[1]].dot(flipped_kernel.T)
...
class NN:
    ...
    def forward_propagate(self, input):  # x ->c ->a -> z -> a ->z -> a -> z -> output
    ...
    def train(self, epoch, inputX, inputY):     #一次計算所有training data 的loss及gradient, 使用gradient和來更新weight
        epochs = list()
        predictions = list()
        losses = list()
        accuracies = list()
    ...
    def train_per_data(self, epoch, inputX, inputY):  #每predict 一筆資料，就根據其loss更新weight
        epochs = list()
        predictions = list()
        losses = list()
        accuracies = list()
...
if __name__ == "__main__":
    ...
    # model_xor = NN(2, 150, 1, "sigmoid", "xor", 0.1)
    # model_xor = NN(2,100,1,"leaky relu", "xor",0.01,"sgd",0);
    # model_xor = NN(2,150,1,"sigmoid", "xor",0.5,"momentum",0.95);

    # using linear model
    model_linear = NN(2, 4, 1,"sigmoid", "linear",0.1);
    input_dataX, input_dataY = model_linear.data_loader.generate_fcn(10000);
    epochs,losses,accuracies,prediction = model_linear.train(30000,input_dataX, input_dataY);
