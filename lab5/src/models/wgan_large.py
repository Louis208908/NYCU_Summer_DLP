...
class Generator(nn.Module):
	...
	def __init__(self, args, device):
		...
		self.main = nn.Sequential(
			...
			nn.ConvTranspose2d(self.args.z_dim + self.args.input_dim * self.args.input_dim, self.args.input_dim * 8, 4, 1, 0, bias=True),
			nn.ConvTranspose2d(self.args.input_dim * 8, self.args.input_dim * 8, 1, 1, 0, bias=True),
			...
			nn.ConvTranspose2d(self.args.input_dim * 4, self.args.input_dim * 4, 1, 1, 0, bias=True),
			...
			nn.ConvTranspose2d(self.args.input_dim * 2, self.args.input_dim * 2, 1, 1, 0, bias=True),
			...
			nn.ConvTranspose2d(self.args.input_dim, self.args.input_dim, 1, 1, 0, bias=True),
...
class Discriminator(nn.Module):
	...
	def __init__(self, args, device):
		...
		self.main = nn.Sequential(
			...
			nn.Conv2d(self.args.n_channel + 1, self.args.input_dim, 4, 2, 1, bias=True),
			nn.Conv2d(self.args.input_dim    , self.args.input_dim, 1, 1, 0, bias=True),
			nn.LeakyReLU(0.2, inplace=True),

			## State size: (self.args.input_dim) x 32 x 32
			nn.Conv2d(self.args.input_dim    , self.args.input_dim * 2, 4, 2, 1, bias=True),
			nn.Conv2d(self.args.input_dim * 2, self.args.input_dim * 2, 1, 1, 0, bias=True),
			nn.InstanceNorm2d(self.args.input_dim * 2, affine=True),
			nn.LeakyReLU(0.2, inplace=True),
			
			## State size: (self.args.input_dim * 2) x 16 x 16
			nn.Conv2d(self.args.input_dim * 2, self.args.input_dim * 4, 4, 2, 1, bias=True),
			nn.Conv2d(self.args.input_dim * 4, self.args.input_dim * 4, 1, 1, 0, bias=True),
			nn.InstanceNorm2d(self.args.input_dim * 4, affine=True),
			nn.LeakyReLU(0.2, inplace=True),
			
			## State size: (self.args.input_dim * 4) x 8 x 8
			nn.Conv2d(self.args.input_dim * 4, self.args.input_dim * 8, 4, 2, 1, bias=True),
			nn.Conv2d(self.args.input_dim * 8, self.args.input_dim * 8, 1, 1, 0, bias=True),
			nn.InstanceNorm2d(self.args.input_dim * 8, affine=True),
			nn.LeakyReLU(0.2, inplace=True), 

			## State size: (self.args.input_dim * 8) x 4 x 4
			nn.Conv2d(self.args.input_dim * 8, 1, 4, 1, 0, bias=True), 
			#nn.Sigmoid()
		)
